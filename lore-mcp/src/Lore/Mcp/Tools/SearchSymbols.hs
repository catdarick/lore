  ...
  { query ::
      Field fieldType Text
        `WithMeta` '[ Description
                        ...
                        \For functions: use action-oriented phrases matching how a developer would name them (e.g. 'publishArticle', 'findArticle'). \
                        \For types and classes: use noun phrases (e.g. 'ArticleStore', 'Article'). \
      ...
      { name = "searchSymbols",
        description =
          Just
            ...
            \When the symbol name is already known, use lookupSymbolInfo for metadata or getDefinitions for source directly if you need it, even if the symbol's module name is unknown; \

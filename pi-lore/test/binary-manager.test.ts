...
import { binaryPath, resolveManagedLoreBinary } from "../src/binary-manager.ts";
...
test("existing valid cached binary is reused without downloading", async () => {
  ...
  const path = await resolveManagedLoreBinary({ projectDir: process.cwd(), cacheRoot, manifest, target, run: await runner(), probe: async () => ({ provider: "stack", ghcVersion: "9.6.5" }), download: async () => { throw new Error("network"); } });
...
test("missing binary is downloaded and installed", async () => {
  ...
  const path = await resolveManagedLoreBinary({ projectDir: process.cwd(), cacheRoot, manifest, target, run: await runner(), probe: async () => ({ provider: "stack", ghcVersion: "9.6.5" }), download: async (_url, dest) => { downloads++; await writeFile(dest, gz); } });
...
test("checksum mismatch fails and leaves no final binary", async () => {
  ...
  await assert.rejects(resolveManagedLoreBinary({ projectDir: process.cwd(), cacheRoot, manifest, target, run: await runner(), probe: async () => ({ provider: "stack", ghcVersion: "9.6.5" }), download: async (_u,d) => writeFile(d, Buffer.from("bad")) }), /SHA-256/);
...
test("wrong binary metadata causes redownload once", async () => {
  ...
  let downloads = 0; await resolveManagedLoreBinary({ projectDir: process.cwd(), cacheRoot, manifest, target, run: await runner(), probe: async () => ({ provider: "stack", ghcVersion: "9.6.5" }), download: async (_u,d) => { downloads++; await writeFile(d, gz); } });
  assert.equal(downloads, 1);
});

test("temporary files are removed after failure", async () => {
  const manifest = manifestFor(gzipSync(script())); const cacheRoot = await makeCache();
  await assert.rejects(resolveManagedLoreBinary({ projectDir: process.cwd(), cacheRoot, manifest, target, run: await runner(), probe: async () => ({ provider: "stack", ghcVersion: "9.6.5" }), download: async (_u,d) => writeFile(d, Buffer.from("bad")) }));

...
import Lore.Tools.Internal.DefinitionSourceRendering (buildDefinitionSourceFiles)
import Lore.Tools.Render.Doc (LoreDoc, SourceFile, ToLoreDoc (toLoreDoc), paragraph, sourceFile)
...
data ResolveInstanceOptions = ResolveInstanceOptions
...
resolveInstance :: (MonadLore m) => ResolveInstanceOptions -> m ResolveInstanceResult
resolveInstance ResolveInstanceOptions {resolveInstanceQuery} =
  withLoadedSession \session -> do
    ...
    case normalizeInstanceQuery resolveInstanceQuery of
      Nothing ->
        pure
